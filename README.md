<div align="center">
  ...
  <img src="https://img.shields.io/badge/OSRM-Map%20Routing-brightgreen?style=for-the-badge" alt="OSRM API" />
...
graph TD
    ...
    B -->|Interactive Map| C[Leaflet.js]
    B -->|Real-world Geometry| D[OSRM API]
    B <-->|Payload Data| E{C++ Backend Engine}
    
    subgraph Advanced Data Structures
    E --> F[Graph Representation<br/>Nodes=Hubs, Edges=Roads]
...
* **Backend Logic (C++)**: Powering the core routing, parsing complex graphs, and executing real-time allocation heuristics. It handles immense data sets with optimal memory efficiency.
* **Frontend GUI (React.js + Vite)**: A premium glassmorphism interface featuring **Framer Motion** animations. 
* **Routing Layer (OSRM)**: Open Source Routing Machine API connects our abstract graph data to actual geographic road networks.
...
### 1. Graph Theory (The City Blueprint)
We modeled the city of Pune as a **Weighted Directed Graph**.
- **Vertices (Nodes)**: Key hubs (e.g., Hinjewadi, Kothrud, Viman Nagar).
- **Edges (Links)**: Connecting roads.
- **Weights**: Instead of just physical distance, weight is calculated as `Cost = Distance × Traffic Factor`.
...
| Feature | Description |
|---------|-------------|
| 🗺️ **Real-Road Geometry** | Routes aren't straight lines. OSRM API fetches the actual curves of Pune's roads. |
...
## 💡 Technical Viva Q&A
> **Q: Why use a Graph instead of an Array for mapping?**  
> *A: Arrays are strictly linear. Urban road networks are inherently non-linear and highly interconnected. Graphs model these complex branching paths, allowing us to perform advanced algorithms like Dijkstra's, which are impossible on standard arrays.*

> **Q: What is the Time Complexity of your pathfinding approach?**  
> *A: By optimizing Dijkstra’s algorithm with a Min-Heap (Priority Queue), we achieve a time complexity of **$O(E \log V)$**, ensuring near-instantaneous route calculation even as the graph grows.*
...
| :--- | :--- | :--- |
| **Daksh** | Algorithm Core | Developed the Graph structures and Traffic-aware Dijkstra’s logic. |
| **Sanskar** | Dispatch Engine | Engineered the Min-Heap for the score-based driver assignment. |
| **Shlok** | Mapping & API | Successfully integrated OSRM Routing and Leaflet.js architecture. |
